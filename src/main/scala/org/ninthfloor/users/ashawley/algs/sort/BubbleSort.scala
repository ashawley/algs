/**
 * BubbleSort.scala --- Recursive bubble sort with pattern matching
 *
...
/**
 * Singleton object providing an bubble sort method.
 *
 * == Integers ==
 * Example interactive session:
 * {{{
 * scala> BubbleSort.sort(List(2, 3, 1))
 ...
 * Compilation:
 * {{{
 * $ scalac ./org/ninthfloor/users/ashawley/algs/sort/BubbleSort.scala
 * }}}
 *
 * Usage:
 * {{{
 * $ scala org.ninthfloor.users.ashawley.algs.sort.BubbleSort < sort.txt
 * }}}
 */
object BubbleSort extends WordReader {
  ...
  def sort(a: List[Int]): List[Int] = {
    def bubble(a: List[Int]): List[Int] = a match {
      case Nil => Nil
      case a :: Nil => a :: Nil
      case a :: b :: aa => if (b < a) b :: bubble(a :: aa)
      else a :: bubble(b :: aa)
    }
    a match {
      case Nil => Nil
      case a :: aa => bubble(a :: sort(aa))
    }
  }
